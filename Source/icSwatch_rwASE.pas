...
uses Classes,SysUtils, Graphics,
  icCore_rw, icSwatch, icSwatch_rwACO ;

type
  //TicAcoColorLoader = function (Stream8 : TStream): TColor of object;

  TicAseConverter = class(TicConverter)
  private
    FAcoColorLoaders : array [0..10] of TicAcoColorLoader;
    ...
    procedure LoadItemFromASE1(Item :TicSwatchItem; Stream : TStream);
    procedure LoadItemFromACO2(Item :TicSwatchItem; Stream : TStream);
  public
    constructor Create; override;
    procedure LoadFromStream(AStream: TStream; ACollection: TCollection); override;
    //procedure LoadItemFromString(Item :TicSwatchItem; S : string);
...
type
  TicAcoItemLoader = procedure (Item :TicSwatchItem; Stream : TStream) of object;
  TicAcoHeader = record
    Version : Word;
    Count   : Word;
  end;
...
constructor TicAseConverter.Create;
begin
  inherited;
...
function TicAseConverter.FAcoCMYKLoader(Stream8: TStream): TColor;
var
  C,M,Y,K : Word;
begin
...
function TicAseConverter.FAcoDummyLoader(Stream8: TStream): TColor;
begin
  Result := clNone; //dummy
end;

function TicAseConverter.FAcoGrayscaleLoader(Stream8: TStream): TColor;
var
  Gray : Word;
  g : byte;
begin
...
function TicAseConverter.FAcoHSBLoader(Stream8: TStream): TColor;
...
function TicAseConverter.FAcoLabLoader(Stream8: TStream): TColor;
var
  L : Word;
  A,B : SmallInt;
begin
...
function TicAseConverter.FAcoRGBLoader(Stream8: TStream): TColor;
var
  R,G,B : Byte;
begin
...
procedure TicAseConverter.LoadFromStream(AStream: TStream;
  ACollection: TCollection);
var
  LCount, i   : Integer;
  LVersion    : TicVersionRec;
  LCollection         : TicSwatchCollection;
  LAseLoader          : TicAcoItemLoader;
begin
  LCollection       := TicSwatchCollection(ACollection);
  be_stream.GStream := AStream;
  BE_ReadDWord; //skip magic word
  AStream.Read(LVersion, SizeOf(TicVersionRec));
  LCount            := BE_ReadDWord;
  ...
  begin
    ...
    try
      for i := 0 to LCount -1 do
        LAseLoader( TicSwatchItem(LCollection.Add), AStream );
    finally
...
procedure TicAseConverter.LoadItemFromASE1(Item: TicSwatchItem;
  Stream: TStream);
var LBlockType, nChars, i : Word;
  LColorSpace : TicVersionRec;
  LBlockLength : Cardinal;
  WS : WideString;
  p : PWideChar;
  LASEColorParser : TicAcoColorLoader;
begin
  ...
  //get color model
  Stream.Read(LColorSpace, SizeOf(TicVersionRec));
...
procedure TicAseConverter.LoadItemFromACO2(Item: TicSwatchItem;
  Stream: TStream);
begin
...
class function TicAseConverter.WantThis(AStream: TStream): Boolean;
var
  LVersion,
  Magic : TicVersionRec;
begin
  be_stream.GStream := AStream;
  AStream.Read(Magic, SizeOf(TicVersionRec));
  AStream.Read(LVersion, SizeOf(TicVersionRec));
...
initialization
  TicSwatchList.RegisterConverterReader('ASE','Adobe Swatch Exchange',0, TicAseConverter);
  //TicSwatchCollection.RegisterConverterWriter('SWA');
end.
