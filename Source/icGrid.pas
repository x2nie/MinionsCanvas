...
{ Graphics32 }
  GR32, GR32_LowLevel,
{ miniGlue }
  icCore_Items, icCore_rw,
{ BroncoImageViewer }
  bivGrid
  ;

type
  TicGridFlow = (ZWidth2Bottom,
                 NHeight2Right,
                 OSquaredGrow,
                 XStretchInnerGrow);
                 
  TicGridChangeLink = class(TicCoreChangeLink);//backward compatibility
  TicGridList = class; //later definition
  TicGridIndex = type Integer; //used for display gradient in property editor



  TicGridItem = class(TicCoreItem)
  protected
  ...
  TicGridCollection = class(TicCoreCollection)
  private
  ...
  TicGridCollectionClass = class of TicGridCollection;


  
  {TicGridList}
  TicGridList = class(TicCoreList)
  protected
    FDescription: string;
  public
  ...
  TicGridListClass = class of TicGridList;
  ...
  TicGrid = class(TbivCustomGrid)
  private
    FChangeLink : TicGridChangeLink;
    FMyItemList : TicGridList;    //destroyable
    procedure CMHintShow(var Message: TMessage); message CM_HINTSHOW;
    
    function GetGridBasedList: TicGridList;
    procedure SetItemList(const Value: TicGridList);
  protected
    FItemList   : TicGridList;      //don't destroy. maybe external
  ...
  TicGridBox = class(TicGrid)
  published
    property ItemList;
  end;
  
  TicGridTheme = class(TbivTheme)
  protected
...
function TicGridItem.CachedBitmap(
...
function TicGridItem.GetHint: string;
begin
  Result := DisplayName;
end;
...
constructor TicGridCollection.Create(AOwner: TComponent);
begin
  Create(AOwner, TicGridItem);
end;

function TicGridCollection.Draw(const AIndex: Integer;
...
begin
  ...
  LItem := TicGridItem(Items[AIndex]);
...
(*procedure TicGridCollection.LoadFromStream(const AStream: TStream);
var
  ...
  //SelfClass           : TicGridCollectionClass;
...
procedure TicGrid.CMHintShow(var Message:TMessage);
    ...
    begin
      ...
      begin
        ...
        LHoverIndex := Self.GetItemAtXY(CursorPos.X, CursorPos.Y);
          ...
          begin
            LItem       := FItemList.Items[LHoverIndex] as TicGridItem;
...
constructor TicGrid.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TicGrid.Destroy;
begin
  FChangeLink.Destroy;
  inherited;
end;

procedure TicGrid.DoCellPaint(ABuffer: TBitmap32; AIndex: Integer;
  ARect: TRect);
begin
...
function TicGrid.GetCellCount: Integer;
begin
  Result := 0;
  if Assigned(FItemList) then
...
function TicGrid.GetGridBasedList: TicGridList;
begin
  Result := FItemList;
end;

function TicGrid.IsSelected(AIndex: Integer): Boolean;
begin
  Result := False;
...
procedure TicGrid.ItemListChangedHandler(Sender: TObject);
begin
...
procedure TicGrid.SetCellCount(const Value: Integer);
begin
  //inherited; //do nothing. it should be a read only property.
end;

procedure TicGrid.SetItemList(const Value: TicGridList);
begin
...
procedure TicGrid.SetSelected(AIndex: Integer);
begin
  if ItemList.IsValidIndex(AIndex) then
  begin
...
procedure TicGridTheme.DoCellPaint(ABuffer: TBitmap32; AIndex: Integer;
  ARect: TRect);
begin
  inherited;
...
procedure TicGridList.ItemPaint(ABuffer: TBitmap32; AIndex: Integer;
  ARect: TRect);
begin
    //demo:
