...
type
  TicCoreViewer = class(TImgView32)
  private
  ...
  TicCoreViewerClass = class of TicCoreViewer;
...
constructor TicCoreViewer.Create(AOwner: TComponent);
begin
  inherited;
  self.Bitmap.ResamplerClassName := 'TKernelResampler';
end;

function TicCoreViewer.GetReaderFilter: string;
begin
  Result := SDefaultFilter;
end;

function TicCoreViewer.GetWriterFilter: string;
begin
  Result := SDefaultFilter;
end;
