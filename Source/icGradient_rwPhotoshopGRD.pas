unit icGradient_rwPhotoshopGRD;
...
{ GraphicsMagic }
  icCore_rw, icGradient;

const
  PS_GRADIENT_FILE_ID = $52474238; // 8BGR

type
  TgrdPhotoshopReader = class(TicConverter)
  private
    procedure LoadColorStopItem(Node: TicGradientStop);
    procedure LoadAlphaStopItem(Node: TicGradientStop);
    function GetStopItemRGB:TColor;
    function GetStopItemCMYK:TColor;
    //function GetStopItemHSB:TColor;
    procedure SetStopItemHSB(Node : TicGradientStop);
  public
    ...
    procedure LoadItemFromStream(AStream: TStream; AGradient: TicGradientItem); reintroduce;
  end;
...
//  LGradients        := TgmGradientCollection(ACollection);
  ...
  for i := 1 to LPShdr.GradientCount do
    LoadItemFromStream(AStream, ACollection.Add as TicGradientItem);
end;

class function TgrdPhotoshopReader.WantThis(AStream: TStream): Boolean;
var
  LFileHeader: TicFileHeader;
begin
...
procedure TgrdPhotoshopReader.LoadAlphaStopItem(Node: TicGradientStop);
begin
...
procedure TgrdPhotoshopReader.LoadColorStopItem(Node: TicGradientStop);
begin
  chunkO.chunk := BE_ReadFlag;       //Objc
  swide        := BE_ReadWideString; //""
  ...
  begin
    ...
    TicGradientItem(Node.Collection.Owner).ColorSpace := chunkO.chunk;
...
procedure TgrdPhotoshopReader.SetStopItemHSB(Node : TicGradientStop);
...
initialization
  TicGradientList.RegisterConverterReader('GRD', 'Photoshop gradient', 0, TgrdPhotoshopReader);
