...
type
  ...
  TicLayerCombinedEvent = procedure (ASender: TObject; const ARect: TRect) of object;
  TicMergeLayerEvent = procedure (AResultPanel: TicBitmapLayer) of object;

  TicLayerList = class(TicCoreCollection)
  private
    //FItems                : TObjectList;
    FSelectedPanel        : TicBitmapLayer;
    FCombineResult        : TBitmap32;
    //FLayerWidth           : Integer; use CombineResult.Width instead
    //FLayerHeight          : Integer;

    FOnLayerCombined      : TicLayerCombinedEvent;
    FOnSelectionChanged   : TNotifyEvent;
    FOnLayerOrderChanged  : TNotifyEvent;
    FOnMergeVisibleLayers : TicMergeLayerEvent;
    FOnFlattenLayers      : TicMergeLayerEvent;

    FPanelTypeCounter     : TicClassCounter;
    FOnLayerChanged: TicLayerChangeEvent;

    
    function GetPanelMaxIndex: Integer;
    function GetSelectedPanelIndex: Integer;
    function GetLayerPanel(AIndex: Integer): TicBitmapLayer;
    ...
    procedure SetLayerPanelInitialName(ALayerPanel: TicBitmapLayer);
    procedure DoLayerChanged(ALayer : TicBitmapLayer);
  public
    constructor Create(AOwner: TComponent); override; 
    destructor Destroy; override;
    procedure Update(Item: TCollectionItem); override;{ COLLECTION. }

    procedure Add(ALayer: TicLayer);
    procedure SimpleAdd(ALayer: TicBitmapLayer); 
    procedure Insert(AIndex: Integer; ALayer: TicLayer);
    ...
    property LayerPanels[AIndex: Integer] : TicBitmapLayer   read GetLayerPanel;
    property SelectedPanel                : TicBitmapLayer   read FSelectedPanel;
    property OnLayerChanged               : TicLayerChangeEvent   read FOnLayerChanged       write FOnLayerChanged; 
    property OnLayerCombined              : TicLayerCombinedEvent read FOnLayerCombined      write FOnLayerCombined;
    property OnSelectionChanged           : TNotifyEvent          read FOnSelectionChanged   write FOnSelectionChanged;
    property OnLayerOrderChanged          : TNotifyEvent          read FOnLayerOrderChanged  write FOnLayerOrderChanged;
    property OnMergeVisibleLayers         : TicMergeLayerEvent    read FOnMergeVisibleLayers write FOnMergeVisibleLayers;
    property OnFlattenLayers              : TicMergeLayerEvent    read FOnFlattenLayers      write FOnFlattenLayers;
  end;
...
function TicNormalLayerPanel_Create(APanelList : TicLayerList;
  AWidth,AHeight: Integer; AColor : TColor32;  AsBackground : Boolean): TicNormalLayerPanel;
begin
  Result := TicNormalLayerPanel.Create(nil);
...
constructor TicLayerList.Create(AOwner: TComponent);
begin
  ...
  FPanelTypeCounter := TicClassCounter.Create;
...
destructor TicLayerList.Destroy;
begin
  //FItems.Clear;
  //FItems.Free;
  FCombineResult.Free;
...
function TicLayerList.GetPanelMaxIndex: Integer;
begin
  Result := Count - 1;
end;

function TicLayerList.GetSelectedPanelIndex: Integer;
var
  i : Integer;
begin
  Result := -1;
...
function TicLayerList.GetLayerPanel(AIndex: Integer): TicBitmapLayer;
begin
  Result := nil;

  if ISValidIndex(AIndex) then
  begin
    Result := TicBitmapLayer(Items[AIndex]);
  end;
end;

function TicLayerList.GetVisbileLayerCount: Integer;
var
  i : Integer;
begin
  Result := 0;
...
function TicLayerList.GetVisibleNormalLayerCount: Integer;
var
  i           : Integer;
  LLayerPanel : TicBitmapLayer;
begin
...
procedure TicLayerList.BlendLayers;
var
  i, j        : Integer;
  LPixelCount : Integer;
  m           : Cardinal;
...
procedure TicLayerList.BlendLayers(const ARect: TRect);
var
  LRect        : TRect;
  i            : Integer;
  x, y, xx, yy : Integer;
...
procedure TicLayerList.DeleteVisibleLayerPanels;
var
  i           : Integer;
  LLayerPanel : TicBitmapLayer;
begin
...
procedure TicLayerList.DeselectAllPanels;
var
  i : Integer;
begin
  if Count > 0 then
...
procedure TicLayerList.SetLayerPanelInitialName(
  ALayerPanel: TicBitmapLayer);
var
  LNumber : Integer;
begin
  if Assigned(ALayerPanel) then
  begin
    if ALayerPanel is TicNormalLayerPanel then
    begin
      if TicNormalLayerPanel(ALayerPanel).IsAsBackground then
      begin
        Exit;
      end;
    end;
...
procedure TicLayerList.Add(ALayer: TicLayer);
begin
  ...
  begin
    ...
    if ALayer is TicNormalLayerPanel then
    begin
      if not TicNormalLayerPanel(ALayer).IsAsBackground then
      begin
    ...
    // first adding
    if (Count = 1) and (ALayer is TicBitmapLayer) then
    begin
      //FLayerWidth  := TicBitmapLayer(APanel).FLayerBitmap.Width;
      //FLayerHeight := TicBitmapLayer(APanel).FLayerBitmap.Height;

      FCombineResult.SetSizeFrom(TicBitmapLayer(ALayer).FLayerBitmap);
    end;
...
procedure TicLayerList.SimpleAdd(ALayer: TicBitmapLayer);
begin
  if Assigned(ALayer) then
  begin
    //FItems.Add(APanel);
...
procedure TicLayerList.Insert(AIndex: Integer;
  ALayer: TicLayer);
begin
  if Assigned(ALayer) then
  begin
...
procedure TicLayerList.Move(ACurIndex, ANewIndex: Integer);
begin
...
procedure TicLayerList.SelectLayerPanel(const AIndex: Integer);
var
  LLayerPanel : TicBitmapLayer;
begin
...
procedure TicLayerList.DeleteSelectedLayerPanel;
var
  LIndex : Integer;
begin
...
procedure TicLayerList.DeleteLayerPanel(AIndex: Integer);
begin
...
procedure TicLayerList.CancelLayerPanel(AIndex: Integer);
var
  LPanel : TicBitmapLayer;
begin
  if (Count = 1) or ( not IsValidIndex(AIndex) ) then
  begin
    Exit;
...
function TicLayerList.CanFlattenLayers: Boolean;
begin
  Result := False;
  ...
  begin
    if Count = 1 then
    begin
      if Self.SelectedPanel is TicNormalLayerPanel then
      begin
        // If the only layer is a Normal layer but not as background layer,
        // we could flatten it as a background layer
        Result := not TicNormalLayerPanel(Self.SelectedPanel).IsAsBackground;
      end;
    end
    else
    begin
...
function TicLayerList.CanMergeSelectedLayerDown: Boolean;
var
  LPrevIndex : Integer;
  LPrevPanel : TicBitmapLayer;
begin
  Result     := False;
...
function TicLayerList.CanMergeVisbleLayers: Boolean;
begin
  Result := FSelectedPanel.IsLayerVisible and
            (GetVisibleNormalLayerCount > 0) and (GetVisbileLayerCount > 1);
end;

function TicLayerList.FlattenLayers: Boolean;
var
  LBackPanel : TicBitmapLayer;
begin
  Result := CanFlattenLayers;
  
  if Result then
  begin
    ///LBackPanel := TicNormalLayerPanel.Create(Self);
...
function TicLayerList.MergeSelectedLayerDown: Boolean;
var
  i             : Integer;
  m             : Cardinal;
...
function TicLayerList.MergeVisibleLayers: Boolean;
var
  LMergedPanel  : TicBitmapLayer;
...
begin
  ...
  begin
    ...
    if FSelectedPanel is TicNormalLayerPanel then
    begin
      LAsBackground := TicNormalLayerPanel(FSelectedPanel).FAsBackground;
    end;

    LMergedPanel := TicNormalLayerPanel_Create(Self,
...
{function TicLayerList.IsValidIndex(const AIndex: Integer): Boolean;
begin
  Result := (AIndex >= 0) and (AIndex < Count);
end;}

function TicLayerList.GetHiddenLayerCount: Integer;
var
  i : Integer;
begin
  Result := 0;
...
procedure TicLayerList.DoLayerChanged(ALayer: TicBitmapLayer);
begin
  if Assigned(FOnLayerChanged) then
    FOnLayerChanged(Self, ALayer);
end;

procedure TicLayerList.Update(Item: TCollectionItem);
// this chance to invalidate the document apperance
begin
  //inherited;
  if Assigned(Item) then
    BlendLayers( TicLayer(Item).FChangedRect )
  else
