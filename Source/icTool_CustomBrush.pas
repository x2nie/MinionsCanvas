...
constructor TicCustomBrush.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TicCustomBrush.Destroy;
begin
  FSourceBitmap.Free;
  FStrokeMask.Free;
...
procedure TicCustomBrush.BrushLine(
...
function TicCustomBrush.GetPaintRect(const AX, AY: Integer): TRect;
begin
  Result := Rect(0, 0, 0, 0);
...
procedure TicCustomBrush.SetSourceBitmap(ABitmap: TBitmap32);
begin
...
procedure TicCustomBrush.SetPaintingStroke(AStrokeMask: TBitmap32);
begin
...
procedure TicCustomBrush.MouseDown(Sender: TicPaintBox;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer;
  Layer: TicLayer);
var
  LRect  : TRect;
  LPoint : TPoint;
  LLayer : TicBitmapLayer;
begin
  LPoint := Sender.ControlToBitmap( Point(X, Y) );

{ Mouse left button down }

  if (Layer is TicBitmapLayer) and (Button = mbLeft) then
  begin
    LLayer := TicBitmapLayer(Layer);
    Felozox          := LPoint.X;
    Felozoy          := LPoint.Y;
...
procedure TicCustomBrush.MouseMove(Sender: TicPaintBox; Shift: TShiftState;
...
begin
  ...
  if FLeftButtonDown and (Layer is TicBitmapLayer)then
  begin
    LLayer := TicBitmapLayer(Layer);
...
procedure TicCustomBrush.MouseUp(Sender: TicPaintBox; Button: TMouseButton;
