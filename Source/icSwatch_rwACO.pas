...
type
  ...
  TicAcoConverter = class(TicConverter)
  private
    ...
    procedure LoadItemFromACO1(Item :TicSwatchItem; Stream : TStream);
...
constructor TicAcoConverter.Create;
begin
  inherited;
...
function TicAcoConverter.FAcoCMYKLoader(Stream8: TStream): TColor;
var
  C,M,Y,K : Word;
begin
  C := BE_ReadWord;
...
function TicAcoConverter.FAcoDummyLoader(Stream8: TStream): TColor;
begin
  Result := clNone; //dummy
end;

function TicAcoConverter.FAcoGrayscaleLoader(Stream8: TStream): TColor;
var
  Gray : Word;
  g : byte;
begin
...
function TicAcoConverter.FAcoHSBLoader(Stream8: TStream): TColor;
...
function TicAcoConverter.FAcoLabLoader(Stream8: TStream): TColor;
var
  L : Word;
  A,B : SmallInt;
begin
...
function TicAcoConverter.FAcoRGBLoader(Stream8: TStream): TColor;
var
  R,G,B : Word;
begin
  R := BE_ReadWord;
...
procedure TicAcoConverter.LoadFromStream(AStream: TStream;
  ACollection: TCollection);
var
  LCount, LVersion, i : Integer;
  LCollection         : TicSwatchCollection;
  LAcoLoader          : TicAcoItemLoader;
begin
  ...
  begin
    ...
    try
      for i := 0 to LCount -1 do
        LAcoLoader( TicSwatchItem(LCollection.Add), AStream );
    finally
...
procedure TicAcoConverter.LoadItemFromACO1(Item: TicSwatchItem;
  Stream: TStream);
...
procedure TicAcoConverter.LoadItemFromACO2(Item: TicSwatchItem;
  Stream: TStream);
begin
...
class function TicAcoConverter.WantThis(AStream: TStream): Boolean;
var
  LVersion : Word;
begin
  be_stream.GStream := AStream;
...
initialization
  TicSwatchList.RegisterConverterReader('ACO','Photoshop Color Swatch',0, TicAcoConverter);
  //TicSwatchCollection.RegisterConverterWriter('SWA','GraphicsMagic Color Swatch',0, TicAcoConverter);
end.
